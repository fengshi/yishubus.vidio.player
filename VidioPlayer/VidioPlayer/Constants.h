//
//  Constants.h
//  VidioPlayer
//
//  Created by shi feng on 13-3-18.
//  Copyright (c) 2013年 com.yishubus.vidioplayer. All rights reserved.
//

#define MAIN                            @"http://www.yishubus.com/"
//#define MAIN                            @"http://192.168.0.108:8080/"

#define MAIN_TITLE_URL                  1
#define MAIN_COLUMN_URL                 2
#define VIDEO_SET_URL                   3
#define CATELOG_URL                     4
#define CATELOG_DETAIL_URL              5
#define SEARCH_URL                      6

#define MAIN_TITLE_HEIGHT               115
#define MAIN_COLUMN_HEIGHT              110
#define MAIN_SECTION_HEIGHT             20
#define MAIN_SECTION_TEXT_SIZE          14

#define MAIN_SECTION_HEIGHT_EMPTY       5
#define MAIN_SECTION_WIDTH_EMPTY        5
#define MAIN_SECTION_IMAGE_WIDTH        70
#define MAIN_SECTION_IMAGE_HEIGHT       MAIN_COLUMN_HEIGHT-MAIN_SECTION_WIDTH_EMPTY*2

#define MAIN_SECTION_BACKGROUND_COLOR   [UIColor colorWithRed:89/255.0 green:89/255.0 blue:89/255.0 alpha:1]

#define MAIN_SECTION_TEXT_COLOR         [UIColor whiteColor]

#define VIDEO_SEGMENTED_HEIGHT          34
#define VIDEO_SEGMENTED_BACKGROUND_COLOR   [UIColor colorWithRed:20/255.0 green:20/255.0 blue:20/255.0 alpha:1]

#define CATELOG_IMAGE_WIDTH             160
#define CATELOG_IMAGE_HEIGHT            80

#define APPID_56                        @"3000001965"
#define SECRET_56                       @"8dfdac2834f3a43f"